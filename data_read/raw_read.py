...
def distorted_inputs(data_dir, batch_size):
    ...
    #reshaped_binary = tf.cast(single_ex.binary_im, tf.float32)
    distorted_image = tf.cast(single_ex.image, tf.float32)
    distorted_dpt_image = tf.cast(single_ex.dpt_image, tf.float32)
    ...
    #distorted_binary = tf.image.central_crop(central_fraction=0.5, image=reshaped_binary)
...
def test_inputs(data_dir, batch_size):
    ...
    distorted_binary.set_shape([height, width, 1])
    ...
    # Generate a batch of images and dpt_images by building up a queue of examples.
    return _generate_image_and_label_batch(float_image, reshaped_dpt_img, distorted_binary,
                                          min_queue_examples, batch_size, shuffle=True)
